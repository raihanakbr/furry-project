...
@onready var sell_panel := $SellPanel as Panel
...
@onready var sell_cost_label := $SellPanel/SellButton/TextureRect/CostLabel as RichTextLabel
...
func _on_button_pressed() -> void:
	if Globals.is_choosing_arcade:
		if !sell_panel.visible:
			for arcade in Globals.arcadeGames:
				var panel = arcade.get_node("SellPanel")
				if panel:
					panel.visible = false
			sell_panel.visible = true
			sell_panel.z_index = 10
		else:
...
func _update_label() -> void:
	...
	sell_cost_label.text = "[b][color=#000000]%s[/color][/b]" % upgrade_cost

func _on_sell_button_pressed() -> void:
	Globals.add_money(upgrade_cost)
	print(Globals.arcadeGames)
	for i in range(len(Globals.arcadeGames)):
		if Globals.arcadeGames[i] == self:
			Globals.arcadeGames.pop_at(i)
			queue_free()
